...
namespace HotelProject.WebUI.Controllers
{
    public class GuestController : Controller
    {
        ...
        public async Task<IActionResult> GuestIndex()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5240/api/Guest");          // Belirtilen adrese istekte bulunuldu.
        ...
        public async Task<IActionResult> AddGuest(CreateGuestDto createGuestDto)
        {
            if (ModelState.IsValid)
            {
                ...
                var responseMessage = await client.PostAsync("http://localhost:5240/api/Guest", stringContent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:5240/api/Guest/{id}");   // Delete işlemi için id parametresi ekledik.
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:5240/api/Guest/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateGuest(UpdateGuestDto updateGuestDto)
        {
            if (ModelState.IsValid)
            {
                ...
                StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
                var responseMessage = await client.PutAsync($"http://localhost:5240/api/Guest/", stringContent);
                if (responseMessage.IsSuccessStatusCode)
                {
                    return RedirectToAction("GuestIndex");
                }
